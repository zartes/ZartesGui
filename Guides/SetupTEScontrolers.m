...
% Last Modified by GUIDE v2.5 20-Aug-2018 13:04:38
...
function ManagingData2Plot(Data,handles)
...
switch size(Data,2)
    ...
    case 3  % TF        
        plot(handles.Result_Axes,Data(:,1),sqrt(Data(:,2).^2+Data(:,3).^2));
        xlabel(handles.Result_Axes,'Freq (Hz)');
        ylabel(handles.Result_Axes,'PSD');
        handles.Result_Axes.XScale = 'log';
        handles.Result_Axes.YScale = 'log';   
    case 4  % I-Vs        
        plot(handles.Result_Axes,Data(:,2),Data(:,4));
...
if handles.FilePlot.Value
    [FileName, FileDir] = uigetfile({'*.txt','Example file (*.txt)'},...
        'Select file','tmp\*.txt','MultiSelect','on');
    if ~isempty(FileName)&&~isequal(FileName,0)
        ...
        if handles.Check_Plot.Value
            if handles.TestPlot.Value  % Current values acquired
            ...
            elseif handles.FilePlot.Value
                if iscell(handles.FileName)
                    for i = 1:length(handles.FileName)
                        ...
                        fclose(fid);
                        ManagingData2Plot(Data,handles);
...
RECT = getrect(handles.Result_Axes);
cmenu = uicontextmenu;
dat.Rect = RECT;
dat.Data = handles.Datos;
h = uimenu(cmenu,'Label','Linear Fitting','UserData',dat);
set(h,'Callback',{@Data2Fitting},'Checked','on');
