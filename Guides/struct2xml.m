...
% Val2str subfunction bugfix by H. Gsenger, 19-9-2011
    ...
    if(nargin == 2)
        ...
        if (isempty(strfind(file,'.xml'))) %#ok<STREMP>
            file = [file '.xml'];
        end
    end
    ...
    if (length(fieldnames(s)) > 1)
        error(['Error processing the structure:' sprintf('\n') 'There should be a single field in the main structure.']); %#ok<SPRINTFN>
    end
