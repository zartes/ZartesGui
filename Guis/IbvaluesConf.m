...
% Last Modified by GUIDE v2.5 12-Nov-2018 11:41:46
...
set(handles.figure1,'Color',[0 110 170]/255,'Position',...
...
if hObject.Value
    set([handles.RemanentOpt handles.Field_Table handles.Field_Table_Str handles.Field_Start],'Enable','on');
    RemanentOpt_Callback(handles.RemanentOpt,[],handles);
else
    set([handles.RemanentOpt handles.Field_Rn handles.Field_Table handles.Field_Table_Str handles.Field_Start],'Enable','off');
end
...
Conf.Field.Values = str2double(handles.AQ_Field.String);
...
% --- Executes on selection change in RemanentOpt.
function RemanentOpt_Callback(hObject, eventdata, handles)
% hObject    handle to RemanentOpt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns RemanentOpt contents as cell array
%        contents{get(hObject,'Value')} returns selected item from RemanentOpt
if hObject.Value == 1
    handles.Field_Rn.Enable = 'on';  
    handles.Field_Table.Data(1:3) = {[]};
    handles.Ibias_Table.Data(1:3) = {[]};
    handles.Temp_Table.Data(1:3) = {[]};
else
    handles.Field_Rn.Enable = 'off';
    handles.Field_Table.Data{1,1} = -500;
    handles.Field_Table.Data{1,2} = 10;
    handles.Field_Table.Data{1,3} = 500;
    
    handles.Ibias_Table.Data{1,1} = 0;
    handles.Ibias_Table.Data{1,2} = 10;
    handles.Ibias_Table.Data{1,3} = 500;
        
    handles.Temp_Table.Data{1,1} = 50;
    handles.Temp_Table.Data{1,2} = 10;
    handles.Temp_Table.Data{1,3} = 80;
end

% --- Executes during object creation, after setting all properties.
function RemanentOpt_CreateFcn(hObject, eventdata, handles)
% hObject    handle to RemanentOpt (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
